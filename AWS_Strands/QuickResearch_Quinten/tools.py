...
def generate_search_url(engine: str, keywords: List[str]) -> str:
    ...
    if engine == "duckduckgo":
        ...
        logger.warning(
            "Google search has known bot detection issues. Consider using 'duckduckgo' instead."
        )
        url = f"https://www.google.com/search?q={encoded_query}"
    else:
        logger.error(
            f"Unsupported engine '{engine}'. Supported: duckduckgo, bing, google"
        )
        raise ValueError(
            f"Unsupported engine '{engine}'. Supported: duckduckgo, bing, google"
        )
    ...
    async def _crawl_with_browser():
        ...
        try:
            ...
            browser_config = BrowserConfig(
                enable_stealth=False, headless=True, verbose=False
            )
            ...
            async with AsyncWebCrawler(config=browser_config) as crawler:
                ...
                if result.success:
                    logger.info(
                        f"Successfully crawled {url}, markdown length: {len(result.markdown)}"
                    )
...
def extract_urls_from_content(content: str, domain_filter: Optional[str] = None) -> str:
    ...
    """
    logger.info(
        f"extract_urls_from_content called: content_length={len(content)}, domain_filter='{domain_filter}'"
    )
    ...
    try:
        ...
        if domain_filter:
            found_urls = {
                url for url in found_urls if domain_filter.lower() in url.lower()
            }
        ...
        logger.info(
            f"Extracted {len(sorted_urls)} URLs"
            + (f" matching filter '{domain_filter}'" if domain_filter else "")
        )
...
def extract_contact_info(content: str) -> str:
    ...
    try:
        ...
        logger.info(
            f"Extracted: {len(contact_info['emails'])} emails, {len(contact_info['phones'])} phones, {len(contact_info['addresses'])} addresses, {len(contact_info['social_media'])} social"
        )
