...
class TestDeepResearchDave:
    ...
    async def test_research_session_flow(self):
        ...
        with patch.object(
            self.dave, "robust_agent_call", new_callable=AsyncMock
        ) as mock_call:
            mock_call.return_value = "Mocked agent response"

            # Start research session
            response = await self.dave.start_research_session(
            ...
            # Synthesize findings
            synthesis_response = await self.dave.synthesize_findings(
                ["test", "analysis"]
            )
...
class TestResearchQuality:
    ...
    def test_comprehensive_research_criteria(self):
        ...
        sources = [
            {
                "url": "https://arxiv.org/framework-comparison",
                "title": "Academic comparison",
            },
            {
                "url": "https://framework-a-docs.com",
                "title": "Official documentation A",
            },
            {
                "url": "https://framework-b-docs.com",
                "title": "Official documentation B",
            },
            {
                "url": "https://github.com/framework-a",
                "title": "Framework A repository",
            },
            {
                "url": "https://stackoverflow.com/framework-questions",
        ...
        session.add_finding(
            "usability", {"metric": "learning_curve", "value": "moderate"}
        )
