...
class ResearchSession:
    ...
    def _assess_credibility(self, source: Dict[str, Any]) -> str:
        ...
        medium_credibility = ["stackoverflow", "github", "medium", "towards", "papers"]
...
class DeepResearchDave:
    ...
    def create_agent(self) -> Agent:
        ...
        try:
            ...
            logger.info(
                f"Deep Research Dave agent created successfully with {len(tools)} tools"
            )
            return agent
    ...
    def _setup_research_tools(self) -> List:
        ...
        if tavily_key:
            try:
                ...
                logger.info(
                    f"✓ Connected to Tavily MCP - {len(tavily_tools)} search tools added"
                )
                for tool in tavily_tools[:3]:  # Log first 3 tools
                    logger.info(
                        f"  - {tool.tool_name}: {tool.mcp_tool.description[:50]}..."
                    )
                ...
                try:
                    ...
                    logger.info(
                        "✓ Added generic MCP client (manual connection required)"
                    )
                except ImportError:
        ...
        if not tools:
            logger.warning(
                "No research tools available - agent will use only LLM knowledge"
            )
        else:
    ...
    def _get_model_config(self):
        ...
        if self.model_provider.startswith("anthropic/") and os.getenv(
            "ANTHROPIC_API_KEY"
        ):
            try:
                ...
                logger.warning(
                    "Anthropic library not available, falling back to Bedrock"
                )
    ...
    async def start_research_session(
        self, topic: str, research_type: str = "comprehensive"
    ) -> str:
    ...
    async def comparative_deep_research(
        self, options: List[str], criteria: List[str] = None
    ) -> str:
